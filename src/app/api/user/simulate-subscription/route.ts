...
const SIMULATED_SUBSCRIPTION_DURATION_DAYS = 30;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const newTokenBalance = currentTokenBalance + SIMULATED_SUBSCRIPTION_TOKEN_GRANT;
    
    const newSubscriptionEndDate = new Date();
    newSubscriptionEndDate.setDate(newSubscriptionEndDate.getDate() + SIMULATED_SUBSCRIPTION_DURATION_DAYS);
    ...
    const transactionDescription = `EduVoice AI Pro Plan (Simulated) activated. ${SIMULATED_SUBSCRIPTION_TOKEN_GRANT} tokens added.`;
    try {
      await databases.createDocument(
        ...
        {
          user_id: userId,
          type: 'simulated_subscription_purchase',
          token_amount_changed: SIMULATED_SUBSCRIPTION_TOKEN_GRANT,
      ...
      console.warn('Failed to log simulated subscription transaction:', logError);
    }

    return NextResponse.json({ 
      message: 'Subscription (Simulated) confirmed successfully. Tokens added and status updated.', 
      newTokenBalance,
      newSubscriptionStatus: 'active',
      newSubscriptionEndDate: newSubscriptionEndDate.toISOString(),
      tokensAdded: SIMULATED_SUBSCRIPTION_TOKEN_GRANT
    });

  } catch (error: any) {
    console.error('Error in simulate-subscription API route:', error);
